...
impl NativeSplitMultimeshContainer {
    ...
    fn _process(&self, #[base] owner: TRef<Spatial>, _delta: f32) {
        if self.visible_range_begin == 0.0 && self.visible_range_end == 0.0 {
            return;
        }
            ...
            if let Some(player) = player.to_object::<Spatial>() {
                ...
                for child in &owner.get_children() {
                    if let Some(child) = child.to_object::<MultiMeshInstance>() {
                        ...
                        let mmi_aabb_center =
                            mmi_aabb.position + mmi_aabb.size / Vector3::new(2.0, 2.0, 2.0);
                        ...
                        if self.visible_range_begin != 0.0 {
                            ...
                            if (d - self.visible_range_begin).abs()
                                < self.visible_range_begin_hysteresis
                            {
                                show = false;
                            }
                        }
                        ...
                        if self.visible_range_end != 0.0 {
                            ...
                            if (d - self.visible_range_end).abs()
                                < self.visible_range_end_hysteresis
                            {
                                show = false;
                            }
                        }
